...
namespace ns_globals
{
	...
	int dtsh()
	{
		return g_ctx.globals.tickbase_shift;
	}
	...
	int get_m_absoluteframetime()
	{
		return m_globals()->m_absoluteframetime;
	}

	int get_m_absoluteframestarttimestddev()
	{
		return m_globals()->m_absoluteframestarttimestddev;
	}
...
void c_lua::initialize()
{
	...
	lua.new_usertype <weapon_t> (crypt_str("weapon"), sol::base_classes, sol::bases<entity_t>(),
		...
		(std::string)crypt_str("get_dt_speed"), & weapon_t::get_max_tickbase_shift
	);
	...
	globals[crypt_str("get_ping")] = ns_globals::dtsh;
	...
	globals[crypt_str("get_m_absoluteframetime")] = ns_globals::get_m_absoluteframetime;
	globals[crypt_str("get_m_absoluteframestarttimestddev")] = ns_globals::get_m_absoluteframestarttimestddev;
...
void c_lua::refresh_scripts()
{
	...
	std::string folder = crypt_str("C:\\FireWare V5\\Scripts\\");
