...
#include "../../features/lagcompensation/anti_lagcomp.h"
...
void __stdcall hooks::hooked_createmove(int sequence_number, float input_sample_frametime, bool active, bool& send_packet)
{
	...
	if (g_ctx.globals.ticks_allowed < 14 && (exploit::get().double_tap_enabled && exploit::get().double_tap_key || exploit::get().hide_shots_enabled && exploit::get().hide_shots_key))
	...
	anti_lagcomp::get().createmove(stage);
