...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags, bool scripts, bool double_color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    ImGui::SetCursorPosX(ImGui::GetWindowSize().x - (double_color ? 54 : 30));
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (PlusButton(label, true, ImVec2(16, 16), "picker", ImColor(col_v4)))
        {
        ...
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_PopupRounding, 4);
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_FramePadding, ImVec2(4, 3));
        PushStyleVar(ImGuiStyleVar_::ImGuiStyleVar_WindowPadding, ImVec2(4, 3));
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float size = window->Size.x;
    ImGui::SetCursorPosY(GetCursorPosY() - 10);
    ImGui::BeginGroup();
    ImGui::SetCursorPosX(5);
    if (ImGui::CustomButton("Copy", "##C", ImVec2(115 * GetStyle().ssadad, 20))) {
        memcpy(c_menu::get().color_buffer, col, components * sizeof(float));
    }
    ImGui::SameLine(-1, 130 * GetStyle().ssadad);
    if (ImGui::CustomButton("Paste", "##P", ImVec2(115 * GetStyle().ssadad, 20))) {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        RenderColorRectWithAlphaCheckerboard(window->DrawList, ImVec2(bb_inner.Min.x + grid_step, bb_inner.Min.y), bb_inner.Max, GetColorU32(col_rgb), grid_step, ImVec2(-grid_step + off, off), rounding, ImDrawCornerFlags_Right);
        window->DrawList->AddRectFilled(bb_inner.Min - ImVec2(5, 0), ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 4, ImDrawCornerFlags_Left);
    }
    else
    {
        ...
        if (col_source.w < 1.0f)
            ...
            window->DrawList->AddRectFilled(bb_inner.Min - ImVec2(5, 0), bb_inner.Max, GetColorU32(col_source), 4);
    }
    RenderNavHighlight(bb, id);
...
bool ImGui::sub_tab(const char* icon, const char* label, bool selected)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 65, pos.y + 40));
    ...
    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 0, bb.Min.y + 0), ImVec2(bb.Min.x + 65, bb.Max.y + 14), ImColor(100, 125, 200, int(225 * it_filled->second)), 4);
    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 17, bb.Max.y + 20), ImVec2(bb.Min.x + 47, bb.Max.y + 24), ImColor(100, 125, 200, int(255 * it_filled->second)), ImDrawFlags_RoundCornersBottom, 3);


    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (15 - label_size.x / 2), bb.Min.y + (26 - label_size.y / 2)), ImColor(text), icon);
...
bool ImGui::subTabs(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    ImVec4 col = ImLerp(ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 0.7f * al }, ImVec4{ 100 / 255.f, 125 / 255.f, 200 / 255.f, 0.5f * al }, t * al);
    ImVec4 col2 = ImLerp(ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 0.4f * al }, ImVec4{ 100 / 255.f, 125 / 255.f, 200 / 255.f, 0.5f * al }, t * al);
    ImVec4 col3 = ImLerp(ImVec4{ 200 / 255.f, 200 / 255.f, 200 / 255.f, 0.4f * al }, ImVec4{ 100 / 255.f, 125 / 255.f, 200 / 255.f, 1.f * al }, t * al);
    if (hovered && !selected)
    {
        col = ImVec4(100 / 255.f, 125 / 255.f, 200 / 255.f, 0.5 * al);
        col2 = ImVec4(100 / 255.f, 125 / 255.f, 200 / 255.f, 0.5 * al);
    }
...
bool ImGui::weapon(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    ImVec4 text = ImLerp(ImVec4{ 100 / 255.f, 100 / 255.f, 100 / 255.f, GetStyle().Alpha }, ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddText(pos + ImVec2(6, 3), ImColor(225, 225, 225, int(255 * alpha)), name);
    ImGui::PopFont();
...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    const ImRect bb(pos, ImVec2(pos.x + 182, pos.y + 40));
    ImGui::ItemSize(size, 0);
    if (!ImGui::ItemAdd(bb, id))
        return false;
    ...
    ImVec4 text = ImLerp(ImVec4{ 100 / 255.f, 100 / 255.f, 100 / 255.f, GetStyle().Alpha }, ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImVec4 text2 = ImLerp(ImVec4{ 100 / 255.f, 100 / 255.f, 100 / 255.f, GetStyle().Alpha }, ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);

    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 13, bb.Min.y), ImVec2(bb.Min.x + 50 + it_filled->second * 6, bb.Max.y), ImColor(100, 125, 200, int(255 * it_filled->second)), 4);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (34 - label_size.x / 2), bb.Min.y + (19 - label_size.y / 2)), ImColor(text), icon);

    ImGui::PushFont(c_menu::get().g_pTabsFont);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 80, bb.Min.y + (10 - CalcTextSize(label).y / 2)), ImColor(text2), label);
    ImGui::PopFont();
