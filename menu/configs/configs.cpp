...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.desync_correction, false, crypt_str("Ragebot.desync_correction"));
	...
	setup_item(&g_cfg.ragebot.desync_roll_resolver, key_bind(HOLD), crypt_str("Ragebot.desync_roll_resolver"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].rage_aimbot_ignore_limbs, false, std::to_string(i) + crypt_str("Ragebot.aimbot_ignore_limbs"));

		setup_item(&g_cfg.ragebot.weapon[i].adaptive_two_shot, false, std::to_string(i) + crypt_str("Ragebot.adaptive_two_shot"));
		setup_item(&g_cfg.ragebot.weapon[i].adaptive_point_scale, false, std::to_string(i) + crypt_str("Ragebot.adaptive_point_scale"));
	}
	...
	setup_item(&g_cfg.antiaim.fake_flick, false, crypt_str("Antiaim.fake_flick"));
	setup_item(&g_cfg.antiaim.flick_tick, 0, crypt_str("Antiaim.fake_flick_tick"));

	setup_item(&g_cfg.antiaim.extended_desync, false, crypt_str("Antiaim.extended_desync"));
	setup_item(&g_cfg.antiaim.extended_desync_int, 0, crypt_str("Antiaim.extended_desync_int"));
	setup_item(&g_cfg.antiaim.extended_desync_type, 0, crypt_str("Antiaim.extended_desync_type"));
	...
	setup_item(&g_cfg.player.player_models_t, 0, crypt_str("Esp.player_models_t"));
	setup_item(&g_cfg.player.player_models_ct, 0, crypt_str("Esp.player_models_ct"));;
	...
	setup_item(&g_cfg.esp.disablepanoramablur, true, crypt_str("Esp.disable_panorama_blur"));
	...
	setup_item(&g_cfg.misc.lightingonshot, false, crypt_str("Misc.lightingonshot"));
...
{
	std::string file = crypt_str("C:\\FireWare\\Configs\\") + config;
	json allJson;
...
{
	std::string file = crypt_str("C:\\FireWare\\Configs\\");
