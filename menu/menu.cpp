...
auto s = ImVec2{}, p = ImVec2{}, gs = ImVec2{ 920, 690 };
...
void Open_lua()
{
	std::string folder = crypt_str("C:\\FirePaste\\");
...
{
	std::string file_path = crypt_str("C:\\FirePaste\\Scripts\\");
...
void c_menu::antiaim()
{
	...
	ImGui::MenuChild("Main", ImVec2(310, 300));
	{
		...
		if (g_cfg.antiaim.fake_flick)
			ImGui::SliderInt(crypt_str("Flick-tick"), &g_cfg.antiaim.flick_tick, 0, 45, false, crypt_str("%d"));
	...
	ImGui::MenuChild("Fake-lag", ImVec2(310, 150));
	{
		ImGui::Spacing();ImGui::Spacing();
		...
		if (g_cfg.antiaim.fakelag)
		{
			...
			ImGui::SliderInt(crypt_str("Limit"), &g_cfg.antiaim.fakelag_amount, 0, 14);
			...
			if (enabled_fakelag_triggers)
				ImGui::SetCursorPosX(3);
			ImGui::SliderInt(crypt_str("Triggers limit"), &g_cfg.antiaim.triggers_fakelag_amount, 0, 14);
...
{
	ImGui::SetCursorPos(ImVec2(250, 53));
	ImGui::MenuChild("Mian", ImVec2(310, 507));
	{
		ImGui::Spacing(); ImGui::Spacing();
	...
	ImGui::MenuChild("Other", ImVec2(310, 430));
	{
		ImGui::Spacing(); ImGui::Spacing();
		if (category == LOCAL)
...
void c_menu::tabs()
{
	...
	ImGui::BeginGroup();
	{
		...
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 245), ImColor(150, 150, 150), "settings, exploit");
		ImGui::PopFont();

		ImGui::SetCursorPosX(27);
		ImGui::SetCursorPosY(230);
		ImGui::PushFont(g_pIconsFont);
		if (ImGui::tab("A", "Rage-Bot", !tab)) tab = 0;
		ImGui::PopFont();

		//antiaim
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 305), ImColor(140, 140, 140), "fake lag, desync");
		ImGui::PopFont();
		...
		//Visuals
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 365), ImColor(150, 150, 150), "esp, dormant");
		ImGui::PopFont();

		ImGui::SetCursorPosX(27);
		ImGui::SetCursorPosY(350);
		ImGui::PushFont(g_pIconsFont);
		if (ImGui::tab("K", "Visuals", tab == 2)) tab = 2;
		ImGui::PopFont();

		//Players
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 425), ImColor(150, 150, 150), "world, warning");
		ImGui::PopFont();

		ImGui::SetCursorPosX(27);
		ImGui::SetCursorPosY(410);
		ImGui::PushFont(g_pIconsFont);
		if (ImGui::tab("M", "Players", tab == 3)) tab = 3;
		ImGui::PopFont();

		//Misc
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 485), ImColor(150, 150, 150), "bunny-hop, clantag");
		ImGui::PopFont();
		...
		//Configs
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 545), ImColor(150, 150, 150), "configs system");
		ImGui::PopFont();
		...
		//Lua
		ImGui::PushFont(g_pTabsFont);
		ImGui::GetWindowDrawList()->AddText(ImVec2(pos.x + 100, pos.y + 605), ImColor(150, 150, 150), "lua system");
		ImGui::PopFont();

		ImGui::SetCursorPosX(27);
		ImGui::SetCursorPosY(590);
		ImGui::PushFont(g_pIconsFont);
		if (ImGui::tab("O", "Lua's", tab == 6)) tab = 6;
		ImGui::PopFont();
	}
	ImGui::EndGroup();
...
void c_menu::draw(bool is_open)
{
...

	{
		...
		if (ImGui::Begin(("FirePaste.cc"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
		{
			s = ImVec2(ImGui::GetWindowSize().x - ImGui::GetStyle().WindowPadding.x * 2, ImGui::GetWindowSize().y - ImGui::GetStyle().WindowPadding.y * 2);
			p = ImVec2(ImGui::GetWindowPos().x + ImGui::GetStyle().WindowPadding.x, ImGui::GetWindowPos().y + ImGui::GetStyle().WindowPadding.y);
			auto draw = ImGui::GetWindowDrawList();
			...
			//logotype
			{
				...
				//ImGui::Image(logo, ImVec2(ImVec2(65.f * dpi_scale, 55.f * dpi_scale)));

				ImGui::PushFont(g_pLoaderFont);
				draw->AddText(ImVec2(p.x + 69, p.y + 62), ImColor(200, 200, 200), "Leak by");
				//draw->AddText(ImVec2(p.x + 111, p.y + 62), ImColor(100, 125, 200), "");
				draw->AddText(ImVec2(p.x + 121, p.y + 62), ImColor(100, 125, 200), "Virtual#5915");
				draw->AddText(ImVec2(p.x + 69, p.y + 32), ImColor(200, 200, 200), "Feel the");
				draw->AddText(ImVec2(p.x + 111, p.y + 32), ImColor(100, 125, 200), "    Paste");
				ImGui::PopFont();

				ImGui::PushFont(Logo_Font);
				draw->AddText(ImVec2(p.x + 66, p.y + 40), ImColor(200, 200, 200), "Fire");
				draw->AddText(ImVec2(p.x + 111, p.y + 40), ImColor(100, 125, 200), "Paste");
				ImGui::PopFont();
			}
			...
			//userlogs
			{
				...
				draw->AddText(ImVec2(p.x + 50, p.y + 155), ImColor(200, 200, 200), version.c_str());
				ImGui::PopFont();

				ImGui::PushFont(g_pinfoFont);
				std::string Beta = "4.1";
				draw->AddText(ImVec2(p.x + 101, p.y + 155), ImColor(100, 125, 200), Beta.c_str());
				ImGui::PopFont();

				ImGui::PushFont(g_pinfoFont);
				std::string welcome = "Welcome back gamer! ";
				draw->AddText(ImVec2(p.x + 50, p.y + 123), ImColor(200, 200, 200), welcome.c_str());
				ImGui::PopFont();
				...
				draw->AddText(ImVec2(p.x + 50, p.y + 139), ImColor(200, 200, 200), Expires.c_str());
				ImGui::PopFont();

				ImGui::PushFont(g_pinfoFont);
				std::string Data = "Never";
				draw->AddText(ImVec2(p.x + 101, p.y + 139), ImColor(100, 125, 200), Data.c_str());
				ImGui::PopFont();
			}
