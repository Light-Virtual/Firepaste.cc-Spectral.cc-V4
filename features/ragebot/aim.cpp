...
void aim::scan(adjust_data* record, scan_data& data, const Vector& shoot_position, bool optimized)
{
    ...
    // adaptive two shot / monolith.paste :3 fix fix fix 
    if (g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].adaptive_two_shot)
        this->adaptive_two_shot();
    ...
    for (auto& point : points)
    {
        ...
        if (fire_data.damage >= current_minimum_damage && fire_data.damage >= best_damage)
        {
            ...
            if (((record->flags & FL_ONGROUND && g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].prefer_safe_points && point.safe) || (record->flags & FL_ONGROUND && point.hitbox >= HITBOX_PELVIS && point.hitbox <= HITBOX_UPPER_CHEST) ||
                ((!(record->flags & FL_ONGROUND) || record) && point.hitbox == HITBOX_HEAD)) && fire_data.damage >= record->player->m_iHealth())
            {
...
std::vector <int> aim::get_hitboxes(adjust_data* record)
{
    ...
    if (g_cfg.player_list.force_body_aim[record->i] || key_binds::get().get_key_bind_state(22))
    {
    ...
    if (g_cfg.ragebot.weapon[g_ctx.globals.current_weapon].hitboxes[6])
    {
...
std::vector <scan_point> aim::get_points(adjust_data* record, int hitbox, bool from_aim)
{
    ...
    else
    {
        ...
        if (!g_cfg.ragebot.weapon[hooks::rage_weapon].adaptive_point_scale)
        {
            scale = bbox->radius * math::random_float(0.25f, 0.75f);
        }
        else if (g_cfg.ragebot.weapon[hooks::rage_weapon].adaptive_point_scale)
        {
            float  v6;
            float pDist = 0.f;
...
void aim::fire(CUserCmd* cmd)
{
    ...
    auto is_valid_hitchance = calculate_hitchance(aim_angle, hitchance_amount);

    if (!is_valid_hitchance)
    {
        auto is_zoomable_weapon = g_ctx.globals.weapon->is_sniper();
    ...
    if (net_channel_info) //p backtrack & гдебэктрэк??
    {
        auto original_tickbase = g_ctx.globals.backup_tickbase;
        auto max_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 0 : 20;
        ...
        if (g_cfg.ragebot.double_tap && g_cfg.ragebot.double_tap_key.key > KEY_NONE && g_cfg.ragebot.double_tap_key.key < KEY_MAX && exploit::get().double_tap_key)
            if (!g_ctx.local()->m_bGunGameImmunity() && !(g_ctx.local()->m_fFlags() & FL_FROZEN) && !antiaim::get().freeze_check && exploit::get().double_tap_enabled && !g_ctx.globals.weapon->is_grenade() && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_TASER && g_ctx.globals.weapon->m_iItemDefinitionIndex() != WEAPON_REVOLVER && g_ctx.globals.weapon->can_fire(false))
                original_tickbase += min(16, max_tickbase_shift);
