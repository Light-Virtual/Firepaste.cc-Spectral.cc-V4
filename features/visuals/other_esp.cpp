	...
	if (g_cfg.misc.holo_panel)
	{
		...
		if (key_binds::get().get_key_bind_state(17))
		{
			...
			if (math::world_to_screen(bone_pos, angle))
			{
				...
				render::get().text(fonts[WATERMARK], angle.x + 110, angle.y - 175, Color(255, 255, 255, 255), 0, "ANTI-AIMBOT | DEBUG");
				...
				render::get().text(fonts[WATERMARK], angle.x + 110, angle.y - 135, Color(255, 255, 255, 255), 0, "DOUBLETAB TYPE : ");

				if (g_cfg.ragebot.double_tap or g_cfg.ragebot.defensive_doubletap)
				{
					render::get().text(fonts[WATERMARK], angle.x + 110 + render::get().text_width(fonts[WATERMARK], "DOUBLETAB TYPE : "), angle.y - 135, Color(117, 219, 15, 255), 0, "INSTANT");
				}
				else
				{
					render::get().text(fonts[WATERMARK], angle.x + 110 + render::get().text_width(fonts[WATERMARK], "DOUBLETAB TYPE : "), angle.y - 135, Color(219, 15, 15, 255), 0, "DEFENSIVE");
				}
