...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (!player_info.fakeplayer && g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum())
	{
		...
		case RESOLVER_LOW_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 35.0f);
			break;
		case RESOLVER_JITTER_FIRST:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + 180.0f);
			break;
		case RESOLVER_JITTER_SECOND:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 180.0f);
			break;
		case RESOLVER_DEFAULT:
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y - 10.0f);
			break;
		}
