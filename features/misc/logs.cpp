...
void eventlogs::paint_traverse()
{
	...
	for (unsigned int i = 0; i < logs.size(); i++)
	{
		...
		render::get().rect_filled(log.x + 5, last_y + log.y + 20, (name_size + 18) / 2, 2, Color(100, 125, 200));
		render::get().rect_filled(log.x + 5 + (name_size + 18) / 2, last_y + log.y + 20, (name_size + 18) / 2, 2, Color(100, 125, 200));
...
void eventlogs::add(std::string text, bool full_display)
{
	...
	m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[ FirePaste.cc ] "));
