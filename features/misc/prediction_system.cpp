...
void engineprediction::predict(CUserCmd* m_pcmd)
{
	...
	if (!prediction_data.prediction_random_seed)
		prediction_data.prediction_random_seed = *reinterpret_cast <int**> (util::FindSignature(("client.dll"), ("A3 ? ? ? ? 66 0F 6E 86")) + 0x1);
	...
	*prediction_data.prediction_random_seed = MD5_PseudoRandom(m_pcmd->m_command_number) & INT_MAX;
	...
	if (!prediction_data.prediction_player)
		prediction_data.prediction_player = *reinterpret_cast <int**> (util::FindSignature(("client.dll"), ("89 35 ? ? ? ? F3 0F 10 48")) + 0x2);
